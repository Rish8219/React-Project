...
const LoginSignup = () => {
    ...
    const [storage, setStorage] = useState([{ name: "", mail: "", pass: "" }])
    ...
    const handleClick = (e) => {
        ...
        if (username && email && password && confirmPassword) {
            if (password === confirmPassword) {
                if (isChecked) {
                    setStorage([...storage, { name: username, mail: email, pass: password }])
                    console.log(storage);
        ...
        <div className="flex justify-center items-center min-h-screen bg-gradient-to-r from-white to-gray-500">
            <div className="bg-white shadow-md rounded-lg p-8 w-full max-w-md sm:max-w-lg md:max-w-xl lg:max-w-2xl xl:max-w-3xl">
                ...
                <form >
                    ...
                    {/* Continue Button */}
                    <button className="w-full h-10 mt-6 cursor-pointer text-white bg-red-500 text-lg rounded-md hover:bg-red-600 transition" onClick={handleClick}>
                        Continue
                    </button>
...
export const Login = () => {
    ...
    const [showPassword, setShowPassword] = useState(false);
    const [email, setEmail] = useState("");
    const [storage, setStorage] = useState([{ mail: "", pass: "" }])

    const handleClick = (e) => {
        e.preventDefault();
        if (email && password) {
            setStorage(email,pass)
        console.log(storage);F
